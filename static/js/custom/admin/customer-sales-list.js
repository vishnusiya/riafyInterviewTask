(function (window) {
  ...
  SHP.salesList = ko.observableArray([]);
  ...
  SHP.getSalesList = function () {
    var csrftoken = SHP.getCookie('csrftoken');
    $.ajax({
      method: 'GET',
      url: '/appshop/customer/sales/list/get',
      ...
      .done(function (d, textStatus, jqXHR) {
        SHP.salesList([]);
        for (var i = 0; i < d.sale_lst.length; i++) {
          SHP.salesList.push(d.sale_lst[i]);
        }  
        SHP.page_count(d.page_count);
        SHP.refreshPagination();
        if (SHP.salesList().length > 0){
